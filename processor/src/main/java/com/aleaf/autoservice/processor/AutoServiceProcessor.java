 ...
 */
package com.aleaf.autoservice.processor;
...
public class AutoServiceProcessor extends AbstractProcessor {
     ...
     */
    private MultiHashMap<String, String> providers = new MultiHashMap<>();

    @Override
    public Set<String> getSupportedAnnotationTypes() {
        Set<String> types = new LinkedHashSet<>();
        types.add(AutoService.class.getName());
        return types;
    }
    ...
    private void processAnnotations(Set<? extends TypeElement> annotations,
        ...
        for (Element e : elements) {
            ...
            AnnotationMirror annotationMirror = Utils.getAnnotationMirror(e, AutoService.class);
            ...
            for (DeclaredType providerInterface : providerInterfaces) {
                TypeElement providerType = Utils.asTypeElement(providerInterface);

                log("provider interface: " + providerType.getQualifiedName());
                log("provider implementer: " + providerImplementer.getQualifiedName());

                if (checkImplementer(providerImplementer, providerType)) {
                    providers.addToList(getBinaryName(providerType), getBinaryName(providerImplementer));
                } else {
    ...
    /**
     * Verifies {@link java.util.ServiceLoader} constraints on the concrete provider class.
    ...
    private Set<DeclaredType> getValueFieldOfClasses(AnnotationMirror annotationMirror) {
        return Utils.getAnnotationValue(annotationMirror, "value")
                .accept(
                        new SimpleAnnotationValueVisitor8<Set<DeclaredType>, Void>() {
                            @Override
                            public Set<DeclaredType> visitType(TypeMirror typeMirror, Void v) {
                                ...
                                // int[].class
                                return Utils.ofSet(Utils.asDeclared(typeMirror));
                            }

                            @Override
                            public Set<DeclaredType> visitArray(
                                    List<? extends AnnotationValue> values, Void v) {
                                Set<DeclaredType> set = new HashSet<>();
                                for (AnnotationValue value : values) {
                                    Set<DeclaredType> ss = value.accept(this, null);
                                    for (DeclaredType s : ss) {
                                        set.add(s);
                                    }
                                }
                                return set;
